...
def fit_graven2013(da, n_jobs=None):
    """
    ...
    if "lat" not in da.dims:
        da = da.expand_dims(lat=1)
    if "lon" not in da.dims:
        da = da.expand_dims(lon=1)

    da_stack = da.stack(coords=["lat", "lon"])

    if da_stack["coords"].size < 10000:
        n_jobs = 1
    ...
    if n_jobs is None:
        n_jobs = Y.shape[0] // 10000
        n_jobs = -1 if n_jobs > 24 else n_jobs
    workers = joblib.Parallel(n_jobs=n_jobs)
    func = joblib.delayed(fit_graven2013_numpy)
    queue = [func(x, y) for y in Y]
    Yhat[~nan_mask] = np.array(workers(queue))
