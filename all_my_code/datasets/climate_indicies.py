...
def _southern_annular_mode_seasonal():
    ...
    df = (
        ...
        .replace("SUM", 11)
        .rename(
            columns={"level_0": "year", "level_1": "month", 0: "southern_annular_mode"}
        )
    )
...
def _southern_annular_mode_annual():
    ...
    time = da.time.values.astype("timedelta64[Y]").astype(
        "timedelta64[D]"
    ) + np.datetime64("0000-01-05")
...
def mauna_loa_xco2():
    ...
    dt = np.timedelta64(14, "D")
    df["time"] = (
        decimal_year_to_datetime(df.decimal_year).values.astype("datetime64[M]") + dt
    )
    ...
    da = da.assign_attrs(
        ...
        units="ppm",
        citation=(
            "Dr. Pieter Tans, NOAA/GML (gml.noaa.gov/ccgg/trends/) and Dr. Ralph "
        ...
        description=(
            ...
            "Data from March 1958 through April 1974 have been obtained by "
            "C. David Keeling of the Scripps Institution of Oceanography (SIO) and "
            "were obtained from the Scripps website (scrippsco2.ucsd.edu). "
            "Monthly mean CO2 constructed from daily mean values Scripps data "
            "downloaded from http://scrippsco2.ucsd.edu/data/atmospheric_co2 "
            "Monthly values are corrected to center of month based on average "
            "seasonal cycle. Missing days can be asymmetric which would produce "
            "a high or low bias. Missing months have been interpolated, for NOAA "
            "data indicated by negative stdev and uncertainty. "
        ),
    )
...
def pacific_decadal_oscillation():
    ...
    df["time"] = pd.to_datetime(df.year + "-" + df.month.str.zfill(2))
    pdo = (
        df.set_index("time")["pdo"]
        .to_xarray()
        .where(lambda x: x > -9)
        .rename("pacific_decadal_oscillation")
    )
    ...
    pdo = pdo.assign_attrs(
        source=url,
        webpage=(
            "https://www.cen.uni-hamburg.de/en/icdc/data/climate-indices/"
            "climate-patterns-of-ocean-and-atmosphere.html#pdo"
        ),
        description=(
            "The Pacific Decadal Oscillation (PDO) is similar to the El-Niño "
            "pattern, but is long-living and describes most of the climate variability "
            "of the Pacific. There appear to be two modes with different spatial and "
            "temporal characteristics of the surface temperature of the North Pacific. "
            "PDO affects the southern Hemisphere and effects on surface climate "
            "anomalies over the central South Pacific, Australia and South America "
            "have been observed. Inter-decadal changes in the Pacific climate have "
            "far-reaching effects on natural systems, including water resources in "
            "America and many marine fisheries in the North Pacific. The mechanisms "
            "that cause the variability of PDO are still unclear. "
            "Pacific Decadal Oscillation is defined as the leading PC of monthly SST "
            "anomalies in the North Pacific Ocean. The extreme phases of the PDO are "
            "classified as warm or cool, derived as the leading principal component of "
            "monthly sea surface temperature anomalies in the North Pacific Ocean "
            "poleward of 20°N (Zhang et. al. 1997). Several independent studies have "
            "shown only two complete PDO cycles in the past century: cool PDO regimes "
            "prevailed from 1890 to 1924 and again from 1947 to 1976, while warm "
            " PDO regimes dominated from 1925 to 1946 and from 1977 to the mid-1990' "
            "(Mantua et al. 2002). The main difference of PDO to ENSO is that PDO is "
            "acting on longer time scales (20-30 years) and affecting mainly the North "
            "Pacific sector. "
        ),
    )
...
def sea_ice_area(time="monthly"):
    """
    Fetches sea ice indices from the Norwegian Meteorological Institute

    Parameters
    ----------
    time : str ['monthly', 'daily']
        The time resolution of the sea ice index

    Returns
    -------
    xarray.Dataset
        Dataset containing the sea ice indicies containing sea ice
        extent and area for the Northern and Southern hemispheres
        All metadata is preserved.
    """

    def process_sea_ice_index(ds):
        hem = ds.area.values.astype(str)
        if "sia" in ds:
            var = "sia"
        elif "sie" in ds:
            var = "sie"

        da = ds[var].rename(f"{var}_{hem}")
        da = da.assign_attrs(
            sic_threshold=ds.sic_threshold.values,
            title=ds.attrs.pop("title", ""),
            description=ds.attrs.pop("summary", ""),
        )

        da = da.drop(["lat", "lon", "sic_threshold"]).to_dataset()

        drop_attrs = ["geospatial_lat_min", "geospatial_lat_max", "keywords"]
        attrs = ds.attrs.copy()
    ...
    url = "ftp://osisaf.met.no/prod_test/ice/index/v2p1/[ns]h/osisaf_*_{time}.nc"
    fs = open_files(url.format(time="monthly"))

    ds = xr.merge([process_sea_ice_index(xr.open_dataset(f.open())) for f in fs])

    hemispheres = xr.IndexVariable(("hemisphere",), ["NH", "SH"])
    sea_ice = xr.Dataset(attrs=ds.attrs)
    sea_ice["sea_ice_extent"] = xr.concat([ds.sie_NH, ds.sie_SH], dim=hemispheres)
    sea_ice["sea_ice_area"] = xr.concat([ds.sia_NH, ds.sia_SH], dim=hemispheres)

    sea_ice = sea_ice.conform.time_center_monthly()
