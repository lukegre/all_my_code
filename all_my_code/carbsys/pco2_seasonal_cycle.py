...
def graven2013_seasonal_fit(da, n_jobs=24):
    ...
    Yhat_clim = (
        Yhat.reshape(lat.size, lon.size, n_yrs, 12).mean(axis=2).transpose(2, 0, 1)
    )

    djf = Yhat_clim[[-1, 0, 1]].mean(axis=0)
    jja = Yhat_clim[[5, 6, 7]].mean(axis=0)

    rmse = (np.square(Yhat - Ynan).mean(axis=1) ** 0.5).reshape(lat.size, lon.size)

    dims = "month", "lat", "lon"
    ds = xr.Dataset(
        coords=dict(month=range(1, 13), lat=da.lat.values, lon=da.lon.values)
    )
    ds["seasonal_clim"] = xr.DataArray(Yhat_clim, dims=dims)
    ds["seas_jja_minus_djf"] = xr.DataArray(jja - djf, dims=dims[1:]).assign_attrs(
        description="JJA - DFJ"
    )
...
def climatology_based_seasonal_fit(da):
    ...
    ds["seas_jja_minus_djf"] = clim.sel(month=[6, 7, 8]).mean("month") - clim.sel(
        month=[12, 1, 2]
    ).mean("month")
...
def prep_data(pco2, dic, alk, temp, salt):
    ...
    ds = xr.merge([pco2, sdic, salk, temp, salt]).assign_attrs(
        description=(
            "s_dissicos and s_talkos were normalised with the long-term " "mean of sos"
        )
    )
...
def revelle_factor(alk, dic):
    gamma = (3 * alk * dic - 2 * dic**2) / ((2 * dic - alk) * (alk - dic))
    return gamma
...
def make_dataset_for_decomposition(spco2, dic, alk, temp, salt, dest="."):
    ...
    gamma = ""  # FIXME: This file will be superceded but until then keep this file
    ...
    for key in variables:
        dat[key] = xr.concat(
            [gamma[key], ds_an.spco2, delta_seas[key], ds_an[key]], "component"
        ).assign_coords(component=component)
...
def _decompose_spco2(input_a, input_b, spco2, dest="."):
    ...
    delta_seas = create_seasonal_cycle_fit_netcdf(
        spco2.to_dataset(name="spco2"), dest=dest
    ).spco2
...
def regions_seasonal_signal(pco2):
    ...
    DJF = rolling_annual_avg(
        quarterly[0::4].assign_coords(time=lambda a: a.time.dt.year + 1)
    )
    JJA = rolling_annual_avg(
        quarterly[2::4].assign_coords(time=lambda a: a.time.dt.year)
    )
    seas = (JJA - DJF).mean("time")

    # getting the seasonal masks with certain conditions met
    NHHL = (((seas < 0) & (seas.lat > 35)) | ((seas > 0) & (seas.lat > 55))) & (
        seas.lat < 65
    )
    NHLL = ((seas > 0) & (seas.lat > 10) & (seas.lat < 50)) | (
        (seas.lat > 10) & (seas.lat < 25)
    )
    SHLL = ((seas < 0) & (seas.lat < -10) & (seas.lat > -55)) | (
        (seas.lat < -10) & (seas.lat > -30)
    )
...
def seaflux_weighting(dest="."):
    ...
    ds = ds[["kw_scaled", "sol_Weiss74", "ice"]]
    ...
    weights = (
        ...
        .sortby("lon")
        .rename(var_str)
        .assign_attrs(description=f"weighting is {var_str}")
    )
