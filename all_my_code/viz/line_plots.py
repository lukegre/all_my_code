...
def time_series(xda, ax=None, label_spacing=0.02, **kwargs):
    ...
    xda.plot(hue=key, ax=ax, add_legend=False, **kwargs)

    t = xda.time.values
    labels_x_pos = t[-1] + (t[-1] - t[0]) * label_spacing

    last_y_value = [line.get_data()[1][-1] for line in ax.get_lines()]
    ...
    for i in range(len(labels)):
        ax.labels += (
            ax.text(labels_x_pos, last_y_value[i], labels[i], c=f"C{i}", **props),
        )
...
def annotate_line(line, xloc, label=None, **kwargs):
    ...
    """
    import numpy as np
    from ..munging.date_utils import datestring_to_datetime

    if label is None:
        label = line.get_label()

    if isinstance(xloc, str):
        xloc = datestring_to_datetime(xloc)
